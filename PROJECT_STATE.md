...
1. 执行第 17 节下一个独立验收单元：stale lock；Night dependency matrix、hard-timeout、reviewer `REVISE` 与 task-source mutation 不重复执行。
2. 保留 `.m9-acceptance/night-dependency-20260824-n4`、`.m9-acceptance/timeout-20260825-t1`、`.m9-acceptance/reviewer-revise-20260826-r1` 与 `.m9-acceptance/task-mutation-20260826-m1` 的状态、phase 日志和 handoff，不代替人类执行 accept/reject。
